...
import { calculateMcqMarksFromAttempt } from '../utils/mcqScoring.js'
  ...
  try {
    ...
    const assessmentsWithDetails = await Promise.all(submittedAssessments.map(async attempt => {
      ...
      if (attempt.assessmentId.assessmentType === 'MCQ') {
        calculatedData = calculateMcqMarksFromAttempt(attempt)
      }
      return {
        ...attempt._doc,
        ...calculatedData,
    ...
    const assessmentsWithCalculatedMarks = await Promise.all(assessments.map(async (attempt) => {
      const mcqResult =
        attempt.assessmentId.assessmentType === 'MCQ'
          ? calculateMcqMarksFromAttempt(attempt)
          : { calculatedMarks: 0 }

      return {
        ...attempt._doc,
        calculatedMarks: mcqResult.calculatedMarks,
        ...
        percentage:
          attempt.status === 'GRADED'
            ...
            : mcqResult.percentage ?? mcqResult.calculatedMarks
      }
    }))
