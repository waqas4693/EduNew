...
import { getData, patchData, postData, postFormData, API_URL } from '../../api/api'
...
const AssessmentReview = () => {
                ...
                {attempt.feedbackFile && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      link.href = `${API_URL}resources/files/ASSESSMENT_FEEDBACK/${attempt.feedbackFile}`
  ...
  const MCQSummary = ({ mcqAnswers = [], mcqs = [] }) => {
    const stats = getMcqAttemptStats(mcqAnswers, mcqs)
    ...
    return (
      <Box>
        ...
        {originalMCQs.map((mcq, index) => {
          ...
          const correctAnswers = getCorrectAnswers(mcq)
          const isCorrect = isMcqAnswerCorrect(mcq, answer)
          const isUnattempted = selectedOptions.length === 0
