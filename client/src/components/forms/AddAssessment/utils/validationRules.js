...
export const validateForm = (formData, sectionId, remainingPercentage) => {
  ...
  // Manual grading roles are only required for QNA and FILE assessments
  if (formData.assessmentType !== 'MCQ') {
    if (!formData.assessor || !formData.moderator || !formData.verifier) {
      errors.push('Please select assessor, moderator, and verifier')
    }
  }
